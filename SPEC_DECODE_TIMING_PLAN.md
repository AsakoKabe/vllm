...
> **⚠️ Scope-решение (2026-07-02): Tier 3 (декомпозиция $T_{SD}$, фит $\alpha_k,\beta_k$, ideal-draft/Bellman,
> offline-харнесс) ИСКЛЮЧЁН — оставляем только метрики, считаемые напрямую из кода.** Разделы про Tier 3 ниже
> оставлены как исторический контекст, но не реализуются. Актуальный статус метрик — `SPEC_DECODE_METRICS.md`.
